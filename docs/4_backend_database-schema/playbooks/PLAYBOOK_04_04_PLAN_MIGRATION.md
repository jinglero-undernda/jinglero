# Playbook 04: Plan Migration

## Purpose

This playbook provides step-by-step instructions for planning incremental schema migrations based on gap analysis. Use this to create migration tasks with clear scope, assess risks, and identify dependencies.

## When to Use This Playbook

- After gap analysis (PLAYBOOK_03)
- Before implementing migration (PLAYBOOK_05)
- When planning Phase 3 refactoring work
- When prioritizing schema improvements
...
```
Group 1: Node Property Gaps
- GAP-001: Missing property on Jingle node
- GAP-002: Incorrect property type

Group 2: Relationship Gaps
- GAP-003: Missing relationship type
- GAP-004: Incorrect relationship direction
```
...
```markdown
## Migration Scope: [Gap Name/Group]
...
1. **Code Dependencies:**
   - What code depends on current schema?
   - What code will be affected by changes?
   - What needs to be updated together?

2. **Schema Dependencies:**
   - What other schema elements depend on this?
   - What schema elements will be affected?
   - What needs to be updated together?
...
**For each migration task, assess risks:**
...
2. **Data Risks:**
   - Risk of losing data during migration
...
**Goal**: Fix significant schema issues
...
- Schema changes: [hours]
- Database migration: [hours]
- Code updates: [hours]
- Testing: [hours]
- Documentation: [hours]
...
1. **Migration Plan** (comprehensive document)
...
```
Create a migration plan for the database schema based on the gap analysis.
...
- PLAYBOOK_05: Implement Migration (use after this)
- PLAYBOOK_02: Validate Requirements (use after implementation)
