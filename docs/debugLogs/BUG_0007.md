...
**Status**: `open` - Systematic audit and fix script required
...
6. ⏳ Document as data quality standard
7. ⏳ Consider scheduled periodic audits
...
- **DO NOT REFACTOR CODE UNTIL REVIEWED AND APPROVED** - This bug log is for documentation and planning only
- The audit script should be designed to be reusable for future data quality checks
- Consider adding relationship direction validation to the relationship creation API to prevent future issues
- The script should preserve all relationship properties when fixing direction (status, createdAt, etc.)
- Special attention needed for relationships with properties that might be direction-dependent (e.g., `APPEARS_IN.order`)
